...
In C++20, as a result of [P0784R7](https://wg21.link/p0784), that changed. Finally we could do allocation during constant evaluation. However, this evaluation was extremely limited. Specifically, any allocation that is happens must be deallocated during *that* constant evaluation.
...
But we still cannot just declare a global `constexpr std::vector`:
...
That's a less than thrilling outcome, and kind of puts the breaks on the whole claim that it's an "easy enough rule to understand." It also doesn't help in establishing a path forward for persistent allocation, which would be even more valuable than simply allowing non-transient allocation.
...
1. for constant destruction, ensure that when running the destructor that no allocation is read as mutable.
2. for constant access, ensure that the allocation is not reachable as mutable in any path.
...
That mutation actually isn't undefined behavior today - the `int` object wasn't created as `const`, so it's simply allowed to be mutated. But I'm not sure worried about this case, because it requires an explicit `const_cast`.
