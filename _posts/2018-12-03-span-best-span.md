...
- Iterator specification. For `span<T>`{:.language-cpp}, the iterators are implementation defined (so as to allow implementations to diagnose bad access, but also arguably for stronger type safety). For `subrange<T*>`{:.language-cpp}, they are `T*`. This is a small win for `subrange`, since I am not sure the added type safety or diagnosability are particularly valuable (I would like to see evidence to the contrary).
