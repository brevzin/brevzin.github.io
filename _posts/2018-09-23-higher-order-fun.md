---
layout: post
title: "Having some fun with higher-order functions"
category: c++
tags:
 - c++
 - c++17
...
3. Otherwise, if the expression `a == b` is well-formed and convertible to `bool`, returns `strong­equality​::​equal` when `a == b` is `true`, and otherwise returns `strong­equality​::​nonequal`.
...
Turns out, there’s an app for that in Boost.HOF. It is called first_of. All we need to do is pass in our functions, which we’ve already created, and we’re… done:
...
This is basically a literal translation of http://eel.is/c++draft/function.objects#func.require-1
