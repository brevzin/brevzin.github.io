...
2. providing an `operator template` that returns some type `R` that is a contiguous range of `std::meta::info` and also providing a constructor from either `(std::meta::from_template_t, R)` or `(R)`. Each reflection returned must be a reflection of a structural type (a reflection of an object is always considered to be structural, that's the reference case).
