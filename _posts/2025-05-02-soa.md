...
> My CppCon 2025 talk, [Practical Reflection](https://youtu.be/ZX_z6wzEOG0), is now online, and goes over this example in more detail and more digressions. Check it out!
{:.prompt-info}
...
So far so good. If I just make everything `public` for [easy debugging](https://godbolt.org/z/d3Eqe8W7f):
...
And with that, we have both `push_back` and `operator[]` [working](https://godbolt.org/z/ae4c885dd):
...
Anyway, that's great, since it lets us just print `v[0]` instead of `std::as_const(v)[0]`. But it doesn't quite print it [the way I'd like](https://godbolt.org/z/bn5bMn4vs):
...
And with all that said and done, let's look at [this program](https://godbolt.org/z/E7aajban7):
...
* Being able to implicitly name enumerators. I don't know what Zig calls this feature, but basically the fact that this works:
  ...
  ```
  After all, is `f(.x)` all that different to support than `f({.x=1})` which already works? The best I can come up with in C++ right now is this syntax, using the proposed [`std::constant_wrapper`](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2025/p2781r8.html):
  ```cpp
