...
And with all that said and done, let's look at [this program](https://godbolt.org/z/We7YEWPK3):
...
struct Pointers;
consteval {
    define_aggregate(^^Pointers, transform_members(^^T, std::meta::add_pointer));
}
```
...
Zig gets me the latter. Maybe in the C++29 time-frame we can come up with some way of doing this too.
...
* This returns `FieldType(field)`, which is short convenience for `@FieldType(T, @tagName(field))`. The `@` functions are all Zig builtins. I'm guessing `@FieldType` is capitalized because it's a type, while `@tagName` is a value.
* `[]FieldType(field)` is a slice. The type `[]T` is what C++ would spell `span<T>`.
...
* Being able to implicitly name enumerators. I don't know what Zig calls this feature, but basically the fact that this works:
  ...
  ```
  After all, is `f(.x)` all that different to support than `f({.x=1})` which already works?
